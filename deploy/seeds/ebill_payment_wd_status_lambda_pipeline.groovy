...
class Lambda extends CommonImpl {
    ...
    private static COMPONENT = "ebill-payment-wd-status-lambda"
    ...
    JenkinsJob start() {
        JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_PATH").withValue("ebill-payment-wd-status-lambda"),
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("ebill_payment_wd_status_lambda_provision.groovy"),
                ])
                .withDryRun(false)
                ...
                .withSonarQube([
                        SonarQube.newInstance()
                                ...
                                .withProperties("sonar.projectKey=org.finra."+ Constants.AGS.toLowerCase() + ":" + COMPONENT + " \n" +
                                        ...
                                        "sonar.coverage.exclusions=**/test/**/*.*, **/dto/*.*, **/entity/*.*,**/exception/*.*, **/constants/*.*, **/repo/*.*,**/config/**/*.*, **/model/**/*.*, **/EbillPaymentWDStatusLambdaApplication.java \n" +
                                        "sonar.exclusions=**/test/**/*.*, **/dto/*.*, **/entity/*.*, **/constants/*.*, **/config/**/*.*, **/model/**/*.*,**/repo/*.*,**/exception/**/*.*, **/EbillPaymentWDStatusLambdaApplication.java \n" +
    ...
    private JenkinsJob deploy(String sdlc) {
        JenkinsJob job = JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("ebill_payment_wd_status_lambda_provision.groovy"),
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_NAME").withValue("RMCS-Ebill-Payment-Wd-Status-Lambda")
        ...
        switch(sdlc) {
            case 'qa02':
                job.withDownstream(corpSysNotificationEngineQA())
                break
        }
    ...
    private JenkinsJob corpSysNotificationEngineQA() {
        JenkinsJob.newInstance()
