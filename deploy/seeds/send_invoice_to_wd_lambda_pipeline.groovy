...
class Lambda extends CommonImpl {
    ...
    private static COMPONENT = "send-invoice-to-wd-lambda"
    ...
    JenkinsJob start() {
        JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_PATH").withValue("send-invoice-to-wd-lambda"),
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("send_invoice_to_wd_lambda_provision.groovy")
                ])
                .withDryRun(false)
    ...
    private JenkinsJob deploy(String sdlc) {
        JenkinsJob job = JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("send_invoice_to_wd_lambda_provision.groovy"),
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_NAME").withValue("RMCS-Send-Invoice-To-WD-Lambda")
                ])
