...
interface Constants {
    ...
    final String PROVISION_LABEL = "provision:20.0.0"
...
class Lambda extends CommonImpl {
    ...
    private static COMPONENT = "ebill-accounting-equation-report-lambda"
    ...
    JenkinsJob start() {
        JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_PATH").withValue("ebill-accounting-equation-report-lambda"),
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("ebill_accounting_equation_report_lambda_provision.groovy")
                ])
                .withDryRun(false)
                ...
                .withSonarQube([
                        SonarQube.newInstance()
                                ...
                                .withProperties("sonar.projectKey=org.finra."+ Constants.AGS.toLowerCase() + ":" + COMPONENT + " \n" +
                                        ...
                                        "sonar.coverage.exclusions=**/test/**/*.*, **/dto/*.*,**/domain/*.*,**/exception/*.*, **/entity/*.*, **/constants/*.*, **/config/**/*.*, **/model/**/*.*, **/EbillAccountingEquationReportLambdaApplication.java \n" +
                                        "sonar.exclusions=**/test/**/*.*, **/dto/*.*,**/domain/*.*,**/exception/*.*, **/entity/*.*, **/constants/*.*, **/config/**/*.*, **/model/**/*.*,**/exception/**/*.*, **/EbillAccountingEquationReportLambdaApplication.java \n" +
    ...
    private JenkinsJob deploy(String sdlc) {
        JenkinsJob job = JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("ebill_accounting_equation_report_lambda_provision.groovy"),
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_NAME").withValue("RMCS-Ebill-Accounting-Equation-Report-Lambda")
                ])
        ...
        switch(sdlc) {
            case 'qa':
            case 'qaint':
                job.withDownstream(corpSysToWorkdayQA())
                break
            case 'qaint02':
                job.withDownstream(corpSysToWorkdayQA())
                break
    ...
    private JenkinsJob corpSysToWorkdayQA() {
        JenkinsJob.newInstance()
