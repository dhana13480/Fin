...
class Lambda extends CommonImpl {
    ...
    private static COMPONENT = "batchfile-process-file-in-chunks-lambda"
    ...
    JenkinsJob start() {
        JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_PATH").withValue("batchfile-process-file-in-chunks-lambda"),
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("batchfile_process_file_in_chunks_lambda_provision.groovy"),
                ])
                .withDryRun(false)
    ...
    private JenkinsJob deploy(String sdlc) {
        JenkinsJob job = JenkinsJob.newInstance()
                ...
                .withAdditionalEnvVars([
                        AdditionalEnvVars.newInstance().withKey("APP_GROOVY").withValue("batchfile_process_file_in_chunks_lambda_provision.groovy"),
                        AdditionalEnvVars.newInstance().withKey("LAMBDA_NAME").withValue("RMCS-Batchfile-Process-File-In-Chunks-Lambda")
