...
tags {
    ...
    costCenter = "RSM302"
    sdlc = "${sdlc}"
    component = "notification-engine-lambda"
}
    ...
    lambdas {
        lambda1 {
            staging {
                ...
                prefix = "RMCS/notification-engine-lambda"
                deployArchive = "rmcs-notification-engine-lambda-${rmcsbuildnumber}.jar"
                generateUniqueLocation = false
            }

            name = "RMCS-Notification-Engine-Lambda-${envMap["sdlcUpper"]}"
            ...
            eventSource {
                type = "SQS"
                sqsName = "ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-NOTIFICATIONS-NOTIFICATION${envMap["queueNameSuffix"]}_QUEUE"
                batchSize = "1"
            }
            environment {
                variables = ["SPRING_PROFILES_ACTIVE": "${sdlc}","EVENT_SOURCE":"arn:aws:sqs:us-east-1:${envMap["accountID"]}:ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-NOTIFICATIONS-NOTIFICATION${envMap["queueNameSuffix"]}_QUEUE"]
            }
