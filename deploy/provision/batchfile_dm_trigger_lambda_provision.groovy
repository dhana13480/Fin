...
tags {
    ...
    costCenter = "RSM302"
    sdlc = "${sdlc}"
    component = "batchfile-dm-trigger-lambda"
}
    ...
    lambdas {
        lambda1 {
            staging {
                ...
                prefix = "RMCS/batchfile-dm-trigger-lambda"
                deployArchive = "rmcs-batchfile-dm-trigger-lambda-${rmcsbuildnumber}.jar"
                generateUniqueLocation = false
            }

            name = "RMCS-Batchfile-DM-Trigger-Lambda-${envMap["sdlcUpper"]}"
            ...
            eventSource {
                type = "SQS"
                sqsName = "ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-RECEIVABLE-DM_MESSAGE${envMap["queueNameSuffix"]}_QUEUE"
                batchSize = "1"
            }

            environment {
                variables = ["SPRING_PROFILES_ACTIVE": "${sdlc}","EVENT_SOURCE":"arn:aws:sqs:us-east-1:${envMap["accountID"]}:ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-RECEIVABLE-DM_MESSAGE${envMap["queueNameSuffix"]}_QUEUE"]
            }
