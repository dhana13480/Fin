...
tags {
    ...
    costCenter = "RSM302"
    sdlc = "${sdlc}"
    component = "receivable-payment-notification-lambda"
}
    ...
    lambdas {
        lambda1 {
            staging {
                ...
                prefix = "RMCS/receivable-payment-notification-lambda"
                deployArchive = "rmcs-receivable-payment-notification-lambda-${rmcsbuildnumber}.jar"
                generateUniqueLocation = false
            }

            name = "RMCS-Receivable-Payment-Notification-Lambda-${envMap["sdlcUpper"]}"
            ...
            eventSource {
                type = "SQS"
                sqsName = "ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-RECEIVABLE-PAYMENT_NOTIFICATION${envMap["queueNameSuffix"]}_QUEUE"
                batchSize = "1"
            }

            environment {
                variables = ["SPRING_PROFILES_ACTIVE": "${sdlc}","EVENT_SOURCE":"arn:aws:sqs:us-east-1:${envMap["accountID"]}:ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-RECEIVABLE-PAYMENT_NOTIFICATION${envMap["queueNameSuffix"]}_QUEUE"]
            }
