...
tags {
    ...
    costCenter = "RSM302"
    sdlc = "${sdlc}"
    component = "receivable-item-lambda"
}
    ...
    lambdas {
        lambda1 {
            staging {
                ...
                prefix = "RMCS/receivable-item-lambda"
                deployArchive = "rmcs-receivable-item-lambda-${rmcsbuildnumber}.jar"
                generateUniqueLocation = false
            }

            name = "RMCS-Receivable-Item-Lambda-${envMap["sdlcUpper"]}"
            ...
            vpcConfig {
                subnetId = subnetList
                securityGroupId = ["${lookup.sg("finra-outbound", envMap["vpcName"])}", "${lookup.sg("finra-support", envMap["vpcName"])}", "${lookup.sg("RMCS-internal", envMap["vpcName"])}", "${lookup.sg("RMCS-shared-service", envMap["vpcName"])}"]
            }
