...
tags {
    ...
    costCenter = "RSM303"
    sdlc = "${sdlc}"
    component = "ebill-finapps-notification-process-lambda"
}
    ...
    lambdas {
        lambda1 {
            staging {
                ...
                prefix = "RMCS/ebill-finapps-notification-process-lambda"
                deployArchive = "rmcs-ebill-finapps-notification-process-lambda-${rmcsbuildnumber}.jar"
                generateUniqueLocation = false
            }

            name = "RMCS-Ebill-Finapps-Notification-Process-Lambda-${envMap["sdlcUpper"]}"
            ...
            eventSource {
                type = "SQS"
                sqsName = "ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-EBILL-DM_FILE_MESSAGE${envMap["queueNameSuffix"]}_QUEUE"
                batchSize = "1"
            }
            environment {
                variables = ["SPRING_PROFILES_ACTIVE": "${sdlc}","EVENT_SOURCE":"arn:aws:sqs:us-east-1:${envMap["accountID"]}:ESMP-X-${envMap["esmpQueueSdlc"]}-${ags}-EBILL-DM_FILE_MESSAGE${envMap["queueNameSuffix"]}_QUEUE"]
            }
